...
class VSDKLoader(object):
    ...
    def get_vsdk_package(cls, version=3.2):
        ...
        """
        version = VSDKTransform.get_string_version(version)

        if version is None:
            raise Exception('No VSD version specified')
    ...
    @classmethod
    def get_fetcher(cls, parent, resource_name):
        """ Get the fetcher corresponding to a resource name in a specified parent
        ...
        """
        fetcher_name = VSDKTransform.get_python_name(resource_name)
