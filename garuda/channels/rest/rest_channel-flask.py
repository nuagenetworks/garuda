...
class GARESTChannel(GAChannel):
    ...
    def listen_events(self, path):
        ...
        events = self.core_controller.get_events_queue(request=ga_request)

        if isinstance(events, GAResponseFailure):
            return self.make_http_response(action=GARequest.ACTION_READ, response=events)
